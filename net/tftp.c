...
#if defined (CONFIG_CMD_TFTPSRV) && defined (UBNT_RECOVERY)
...
#define DEVICE_DEVINFO_FILE "devinfo"
static ulong	TftpLastTimer;
static int		TftpRecoveryMode=0;
extern ulong rec_check_image (ulong addr, ulong data_len);
#endif
...
#if defined (CONFIG_CMD_TFTPSRV) && defined (UBNT_RECOVERY)
#define STATE_BADFW 8
#define STATE_DATA_SEND	9
#endif
...
#if defined (CONFIG_CMD_TFTPSRV) && defined (UBNT_RECOVERY)
static char device_info[TFTP_BLOCK_SIZE*2]; /*storage for text device info*/
extern int recovery_duration;
extern const char *get_dev_name(void);
extern int get_dev_rcvr_dur(void);

static char *TftpGetDevinfo(void) {
	int str_idx = 0;
	const char *dev_name = get_dev_name();
	int rcvr_dur = get_dev_rcvr_dur();
	str_idx+=sprintf(&device_info[str_idx],"platform=%s\n",(dev_name)?dev_name:"-1");
	str_idx+=sprintf(&device_info[str_idx],"duration=%d\n",rcvr_dur);
	return device_info;
}
#endif
...
#define TFTP_SRV_FW_CHECK_FAIL	"Firmware check failed"
		...
		len = pkt - xp;
		break;
	case STATE_DATA_SEND: /* send data to client */
		if ((TftpBlock - 1) * TFTP_BLOCK_SIZE > TftpFileSize) {
			printf("Client requested data block outside available data! (#%lu)\n",
				TftpBlock);
			return;
		}
		xp = pkt;
		...
		*s++ = htons(TFTP_DATA);
		*s++ = htons(TftpBlock);
		if (TftpFileSize < TFTP_BLOCK_SIZE)
			len = TftpFileSize;
		else if ((TftpBlock - 1) * TFTP_BLOCK_SIZE >= TftpFileSize)
			len = TftpFileSize % TFTP_BLOCK_SIZE;
		else
			len = TFTP_BLOCK_SIZE;
		memmove((uchar *)s, (void *)(TftpFileAddr + (TftpBlock - 1) * TFTP_BLOCK_SIZE), len);
		pkt = (uchar *)s;
		len += pkt - xp;
		break;
#endif
	    ...
	    unsigned len)
{
	...
	unsigned total_len;
	ulong boot_addr;
	char *local_args[3] = {"bootoctlinux", "0x0000000000000000", NULL};
#endif
	...
	case TFTP_RRQ:
		...
		if (!strcmp(DEVICE_DEVINFO_FILE, tftp_filename)) {
			/* send whole flash dump */
			char *d_info = TftpGetDevinfo();
			TftpFileAddr = (ulong)d_info;
			TftpFileSize = strlen(d_info);
		} else {
			/* just ignore */
		...
		TftpState = STATE_DATA_SEND;
		printf("\nSending <%s> (0x%08lX@0x%08lX) to %pI4:%d\n",
	...
	case TFTP_DATA:
		...
		if(TftpRecoveryMode) {
			if (TftpBlock == 0) {
				...
				if (((get_timer(TftpLastTimer)) > (CONFIG_SYS_HZ/2)) || (len < TFTP_BLOCK_SIZE)) {
					printf("Received %lu bytes\r",
						(TftpBlock -1 ) * TFTP_BLOCK_SIZE + len);
					TftpLastTimer = get_timer(0);
					show_led_state(LED_STEP_NEXT);
				}
			}
		}
		else
		...
		if (TftpRecoveryMode) {
			if (len < TftpBlkSize) 
			{
				printf("\n");
				fw = (uchar *)load_addr;
				total_len = (TftpLastBlock - 1) * TFTP_BLOCK_SIZE + TftpBlockWrapOffset + len;
				boot_addr = rec_check_image((ulong)fw,(ulong)total_len);
				if(boot_addr) {
					TftpSend(); //Firmware is OK. Send confirmation
...
#if defined (CONFIG_CMD_TFTPSRV) && defined (UBNT_RECOVERY)
	if (TftpRecoveryMode) {
		show_led_state(LED_RECOVER_RUN);
