...
int _do_urescue(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int net_res;
	int cust_ip = 0;
	setenv("useraw", NULL);
	...
	while (argc > 1) {
		...
		if (!(strcmp(argv[argc-1], "-s"))) {
			printf("Trying USB storage first\n");
			rec_usb(cmdtp);
		}
		argc--;
	}
	...
	if (!cust_ip)
		setenv("ipaddr",CONFIG_TFTP_RECOVERY_DEFAULT_IP);
	setenv("tftptimeout","1000");
	setenv("ethact", CONFIG_TFTP_RECOVERY_INTERFACE);

	net_res = netboot_common(TFTPSRV, cmdtp, argc, argv);
	setenv("urescue",NULL);
	return net_res;
}

U_BOOT_CMD(
	urescue,	3,	1,	_do_urescue,
	"urescue\t- start TFTP server and wait for firmware\n",
	"[-e] use environment ip\n"
	"[-s] try to load image from USB storage first\n"
	"[-r] use raw image \n"
);
#endif
